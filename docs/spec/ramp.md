...
> 源码：`../../inc` + `src/ramp.cpp` ｜ 血缘：cyclotron `FOC_MATH_SPEC.md` §3.2（已退役）；精确定义**以本文为准**
...
- 无 dt 守卫（与 LPF 同）：调用方保证 `dt > 0`；`dt = NaN` 时两个比较均为假 → **原样透传 cmd**（静默，见 docs/TODO.md T8）。
...
- PID 输出斜坡（`max_rate_out_ > 0` 时启用；PID 的 `0` = 关闭，**与 `Ramp(0)` = 冻结不同层**：PID 在构造期把“关闭”归一化为无上限速率，所以本原语自己的 0 语义始终是“冻结”）。
