...
| T3 | P1 | **`i_saturated_` 语义偏直觉**：表示"本拍 I 项**候选值**被削"，不是"积分器饱和" | 实测：积分分离冻结（error≫thresh）时候选 1.0 被削到 0.5 → `i_saturated_=1` 而 `integral_=0` | 是（M2/F3 要拿它当判据）→ **本轮已把语义写进 spec**，故不再阻塞 | ✅ 已按此落地（v0.1.1）：`i_saturated_` 只报被采纳的钳位，并新增 `i_frozen_` 分离出口 |
...
| T7 | P3 | **版本只活在 `CMakeLists.txt` 与 CHANGELOG**：下游拷走文件后无法自证版本 | 无 `install()` / CPack / 版本宏（grep = 0） | 否 | ✅ 已补（2026-09-17）：`inc/ctl/version.hpp` 为版本唯一事实源；可选 install 规则未做（不阻塞） |
