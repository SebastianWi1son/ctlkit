...
- **D-1 `0` 语义统一（M0 前置，阻塞 D3/A3/L2）**
  现状两套语义并存：`limit_out_=0 → 输出恒 0` vs `thresh_i_sep_/max_rate_out_/d_filter_Tf_=0 → 禁用`。
  方案：限幅类字段统一为"`<=0` = 不限幅"，行为变更点仅在 `limit_out_=0` 的既有配置
  （当前 FOC 电流环显式配置了非零限幅，实测不受影响），配置迁移点写入 SPEC 变更记录。
