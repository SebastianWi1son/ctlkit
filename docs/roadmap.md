...
| A1 外部微分注入 | L1/L2 | **M2（✅ 已落地）** | 经 `PIDPorts.meas_dot_` 注入，直接替代环内差分；`calc` 签名冻结（尾部默认参数端口） |
...
| **M2 品质** | A1 外部微分（✅ 已落地）+ F3 条件积分（含 B1 评估）+ F1 前馈 + F2 目标 LPF | 饱和恢复无迟滞；前馈使阶跃跟踪误差显著下降；各特性默认关闭时与 M1 输出逐位一致 |
...
1. **黄金向量**：固定输入序列（阶跃/斜坡 + dt 扰动）下逐拍比对。
   **已建立**（`oracle/` → `tests/golden/*.csv` → `tests/golden_test.cpp`，容差逐例推导、条数以目录为准）；
...
| L1 核心 | PID 公式冻结 + 注入口（set_integral / set_gains）+ 观测出口（get_state / status / input_fault）+ 端口（PIDPorts） | ✅ **完整**（含 A1 外部微分注入） |
| L2 增强 | 条件积分 · 前馈 · 目标 LPF · 2-DOF · 非对称限幅 · 分离迟滞 | ⬜ 0%（设计就绪；全部按“0 = 默认关闭、只增不改”预留） |
| L3 系统 | 级联协议 · 增益调度 · 自整定 | ⬜（远期） |

里程碑：M0 ✅ · M1 ✅ · M2 ⬜（部分：A1 ✅，其余待触发）· M3 ⬜（场景驱动）· M4 ⬜（前瞻）。
...
| 地基要求 | 状态 |
|---|---|
| 组件可用且被验证 | ✅ 5 组件 · 黄金向量全绿 · smoke · 判别力实测（改坏必红） |
...
| 版本与兼容承诺 | ✅ v0.1.0 冻结 + 兼容政策（minor 只增不改；breaking → major；见 README 兼容政策节） |
| 许可 | ✅ MIT（LICENSE 为 MIT 原文；第三方范围说明在 README 许可节） |
| 下游接入演练 | ✅ 两个仓（2026-09-17）：cyclotron/foc · lunokhod/actuator/wheel —— 均 vendor + 转发头，三条验收 + 行为逐字节复现，记录见 `docs/migration_WORK.md` §4 |

**判定**：🟢 **地基全部达标（2026-09-17）** —— 三项收尾（版本 / 许可 / 接入演练）已完成，下游验证做了两个仓
（lunokhod 一侧用的是平铺具名字段写法，正好验证了迁移的机械成本）。此后只按「只增不改」拓展；
未发布增量（`status().i_frozen_` · `PIDConfig` 具名链式设置器）在 dev 分支攒着，冻结时 bump 版本 + merge 回 main。
